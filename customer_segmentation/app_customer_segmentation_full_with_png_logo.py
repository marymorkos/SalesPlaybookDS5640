    ...
    if predict:
        ...
        with st.expander("📈 Cluster Visualization"):
            try:
                ...
                all_centroids = model.cluster_centers_
                pca_centroids = pca.fit_transform(all_centroids)
                input_pca = pca.transform(scaler.transform(input_df))

                fig, ax = plt.subplots()
                ax.scatter(pca_centroids[:, 0], pca_centroids[:, 1], c='gray', label='Cluster Centroids')
                ax.scatter(input_pca[:, 0], input_pca[:, 1], c='red', label='Input Customer', marker='X', s=100)
                ax.legend()
                ax.set_title("PCA Projection of Customer Segment")
                st.pyplot(fig)
            except Exception as e:
                st.warning(f"Unable to generate plot: {e}")
...
elif page in ["Deal Outcome Predictor", "Pipeline ETL Overview", "Team Insights"]:
